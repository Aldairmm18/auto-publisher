# Migrar generación de texto de Gemini a Claude (Anthropic)

## CONTEXTO
La API de Gemini tiene la cuota agotada y no se puede usar. Vamos a reemplazar Gemini por Claude (Anthropic) para la generación de texto. El proyecto está en `C:\Users\Aldair Murillo\auto-publisher\backend\`.
...
```
# Anthropic (Claude AI)
ANTHROPIC_API_KEY=PEDIR_AL_USUARIO
```

IMPORTANTE: Después de hacer el cambio, preguntarle al usuario por su ANTHROPIC_API_KEY para completar el .env. La key se obtiene en https://console.anthropic.com/settings/keys
...
Buscar en TODOS los archivos de `backend/` si queda alguna referencia a "gemini", "genai", o "GEMINI". Si encuentras alguna, reemplázala por la equivalente de Anthropic/Claude.

Archivos a revisar:
- backend/config.py
- backend/services/ai_text.py
- backend/main.py
- backend/.env.example
- backend/.env

En `backend/main.py` NO debería haber referencias directas a Gemini porque solo importa de `services/ai_text.py`, pero verifica por si acaso.
...
```bash
git add -A
git commit -m "refactor(ai): migrar generación de texto de Gemini a Claude (Anthropic)"
...
| config.py | `GEMINI_API_KEY` → `ANTHROPIC_API_KEY` |
| services/ai_text.py | Reescrito completo: usa `anthropic.Anthropic` + modelo `claude-sonnet-4-20250514` |
| .env.example | Variable cambiada a `ANTHROPIC_API_KEY` |
